...
LIBERO デモデータ (HuggingFace Hub or hdf5変換)
  ...
  - 観測: agentview画像 + hand画像 + state 8D
  - 言語指示: タスク名テキスト
...
| `robot0_eef_pos` + `robot0_eef_quat` + `robot0_gripper_qpos` | `proprio` | 8D |
| タスク名テキスト | `instruction` | str |
| EEF delta + gripper | `actions` | 7D |

## 設定ファイル

`configs/libero_spatial.yaml`:
...
for task_idx in range(num_tasks):  # 10タスク
    env = OffScreenRenderEnv(bddl_file=full_bddl_path)
    successes = 0

    for episode in range(n_episodes):  # 10エピソード
        obs = env.reset()
        # LIBERO提供の初期状態を設定
        init_state = load_init_state(task_idx, episode)
        obs = env.set_init_state(init_state)

        for step in range(max_steps):  # 最大300ステップ
            # LIBERO観測 → VLAPolicy入力に変換
            batch = convert_obs_to_batch(obs, task_language)
...
def convert_obs_to_batch(obs, instruction):
    ...
    # Proprio: EEF pos(3) + quat(4) + gripper(2→1に集約) = 8D
